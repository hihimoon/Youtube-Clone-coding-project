<%@page import="frontweb.vo.YoutubeMember"%>
...
<body>
	...
	<%
	YoutubeMember member = new YoutubeMember(id, password, passwordcheck, name, birth_str, email);
	/* int insCnt = dao.insertYoutubeMember(); */
	int insCnt = dao.insertYoutubeMember(new YoutubeMember("id", "password", "passwordcheck", "name", "date_str", "email"));
	%>
	
	<!-- int insCnt = dao.insertYoutubeMember(new YoutubeMember("id", "password", "passwordcheck", "name", "birth_str", "email")); -->
		...
		<form name="fo" method="post" action="signupAction.jsp" id="signup-form">
			...
			<div class="pwChackWrapper">
				<label for="passwordcheck">비밀번호 확인 <input class="formStyle" type="password" name="passwordcheck"
					id="password-retype" placeholder="비밀번호 확인" value=""></label>
